...
ssh $MY_UGRAD "scp paulproteus@fomalhaut.acm.jhu.edu:/mnt/paul/space/$file /tmp/$file ; hostname ;   ~/bin/7za e -so /tmp/$file | ( cd  ~/dnet/collab/code/ ; python create_title_index.py ) | gzip > $RESULTS/${file%.7z}.gz " 2>&1 | gzip > $CHATDIR/$file-chat  &
