...
( ssh $MY_UGRAD "scp $SOURCE_MACHINE:$SOURCE_PATH/$file $WORKDIR/$file ; hostname ;   ~/bin/7za e -so $WORKDIR/$file | ( cd  ~/dnet/collab/code/ ; python paragraphs2sentences.py  file:///dev/stdin ) | ~/bin/7za a -si $RESULTS/$file ; scp $RESULTS/$file $SOURCE_MACHINE:$DEST_PATH "  ; echo $file finished on $MY_UGRAD ) & 
