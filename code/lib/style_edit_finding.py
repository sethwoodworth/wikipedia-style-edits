...
def improve_old(old, new):
    better = old # hopefully get better!
    
    ## Get min edit distance operations
...
def is_hunk_style_edit(hunk):
    ...
    for old in hunk.olds:
        old_tokens.extend([s.lower() for s in t.filter_line(old)])
    new_tokens = []
    for new in hunk.news:
        new_tokens.extend([s.lower() for s in t.filter_line(new)])
